...
while i < len(SOF_list):
    for EOF in EOF_list:
        ...
        filename="file"+ str(filecounter) + "_"+str(SOF_list[i])+"_"+str(EOF + 18)+".docx" 
        carve_obj=open(filename,'wb')
        carve_obj.write(subdata)
        carve_obj.close()
        i=i+1  
    ...
    if check == 0:
        if (int(SOF) > previousfooter):
            try:
                ...
                filename="file"+ str(filecounter) + "_"+str(SOF)+"_"+str(size)+"bmp" 
                carve_obj=open(filename,'wb')
                carve_obj.write(subdata)
                carve_obj.close()  
                print(carve_filename)
...
for SOF in SOF_list:
    for EOF1 in EOF_list1:
            j=0
            for EOF2 in EOF_list2:
                if int(EOF1)<int(EOF_list2[j]) and int(EOF2) < int(SOF):
                    y=0
                    for EOF3 in EOF_list3:
                        if int(EOF2)<int(EOF_list3[y]) and int(EOF3) < int(SOF):
                            z=0
                            for EOF4 in EOF_list4:
                                if int(EOF3)<int(EOF_list4[z]) and int(EOF4) < int(SOF):
                                    if int(EOF_list4[z+1]) < int(SOF) 
                                    subdata=data[SOF:EOF4]
                                    carve_filename="file"+ str(filecounter) + ".pdf Start Offset: 0x" + str(SOF)+ " End Offset: 0x" + str(EOF4)
                                    filename="file"+ str(filecounter) + "_"+str(SOF)+"_"+str(EOF4)+"pdf" 
                                    #carve_obj=open(filename,'wb')
